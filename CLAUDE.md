...
- `frontend/public/.well-known/assetlinks.json` must contain the SHA-256 of the key in `~/.android-signing/kipato.keystore` and the package id `com.gnm7208.kipato`. If the key ever changes, regenerate this file or the Android app falls back to a browser tab with a URL bar.
- `frontend/public/privacy.html` is the store-facing privacy policy; keep it truthful when new data is collected.
- The Android (TWA) project is generated by Bubblewrap from `../store-packaging/kipato/twa-manifest.json`; rebuild with `bubblewrap update && bubblewrap build` there after a manifest/icon change, and bump `appVersionCode` for every store upload.
